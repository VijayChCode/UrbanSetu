  ...
  return (
    <>
      <tr className="hover:bg-blue-50 transition align-top">
        ...
        {/* Add dustbin icon for past appointments (user side) */}
        <td className="border p-2 text-center">
          {!(isUpcoming) && (isBuyer || isSeller) && (
            <button
              className="text-gray-400 hover:text-red-700 text-xl"
              onClick={handlePermanentDelete}
              title="Remove past appointment from table"
              style={{ opacity: 0.7 }}
            >
              <FaTrash className="group-hover:text-red-900 group-hover:scale-125 transition-all duration-200" />
            </button>
          )}
        </td>
