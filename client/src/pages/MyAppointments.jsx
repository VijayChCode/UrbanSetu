    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-100 py-10 px-2 md:px-8">
      ...
      {showChatModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-slideUp flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white rounded-t-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900">Appointment Chat</h3>
                  <p className="text-sm text-gray-600">Chat with the other party</p>
                </div>
              </div>
              <button
            ...
            {/* Message List */}
            <div className="flex-1 max-h-96 overflow-y-auto space-y-2 mb-4 pr-2 p-6 bg-gray-50 rounded-b-2xl">
              {comments.length === 0 ? (
                ...
                return (
                  <div key={c._id || index} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`rounded-xl px-4 py-2 text-xs shadow ${isMe ? 'bg-blue-100 text-blue-800' : 'bg-white text-gray-800'} max-w-[80%] border ${isMe ? 'border-blue-200' : 'border-gray-200'}`}> 
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold">{isMe ? "You" : c.senderEmail}</span>
            ...
            {/* Input Area */}
            <div className="flex gap-2 p-6 border-t border-gray-100 bg-white rounded-b-2xl">
              <input
                type="text"
                className="flex-1 px-4 py-2 border rounded-xl text-xs focus:ring-2 focus:ring-blue-200"
              ...
              <button
                ...
                className="bg-blue-500 text-white px-4 py-2 rounded-xl text-xs hover:bg-blue-600 transition disabled:opacity-50 flex items-center gap-2"
              >
                <FaPaperPlane /> Send
              </button>
              <button
