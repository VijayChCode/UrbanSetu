...
export default function ImmersiveTour({ isOpen, onClose, tourImages, propertyName }) {
  ...
  // If no images, don't even try to render the AnimatePresence content
  if (!tourImages || tourImages.length === 0) return null;
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Main 360 Viewer */}
          <div className="flex-1 relative overflow-hidden bg-gray-950" onClick={(e) => e.stopPropagation()}>
            ...
            {loading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-950 z-40">
            ...
            {!loading && tourImages.length > 1 && (
              <>
                <button
                  onClick={handlePrev}
                  className="absolute left-6 top-1/2 -translate-y-1/2 z-50 p-4 bg-black/50 hover:bg-blue-600 backdrop-blur-md text-white rounded-full transition-all border border-white/20 hover:scale-110 active:scale-95 group hidden md:flex shadow-2xl"
                >
                  <FaChevronLeft className="group-hover:-translate-x-1 transition-transform" />
                </button>
                <button
                  onClick={handleNext}
                  className="absolute right-6 top-1/2 -translate-y-1/2 z-50 p-4 bg-black/50 hover:bg-blue-600 backdrop-blur-md text-white rounded-full transition-all border border-white/20 hover:scale-110 active:scale-95 group hidden md:flex shadow-2xl"
                >
          ...
          {/* Header Overlay - MOVED AFTER VIEWER TO BE ON TOP */}
          <div className="p-4 md:p-6 flex items-center justify-between bg-gradient-to-b from-black/90 via-black/40 to-transparent absolute top-0 left-0 right-0 z-[1000] pointer-events-none">
            ...
            <div className="flex items-center gap-3 pointer-events-auto">
               <button
                onClick={handleClose}
                className="p-3 md:p-4 bg-white/10 hover:bg-red-500 text-white rounded-full transition-all duration-500 border border-white/20 group hover:scale-110 active:scale-90 shadow-2xl backdrop-blur-xl"
                title="Close Tour"
              >
          ...
          <div className="p-6 bg-gradient-to-t from-black via-black/80 to-transparent absolute bottom-0 left-0 right-0 z-[1000]" onClick={(e) => e.stopPropagation()}>
          ...
          <style>{`
            .immersive-tour-container { background-color: #030712 !important; }
            .psv-navbar {
