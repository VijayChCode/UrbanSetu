...
import { FaCompass, FaPlay, FaBrain, FaExclamationTriangle, FaCheckCircle, FaLightbulb, FaInfoCircle, FaShieldAlt } from "react-icons/fa";
...
export default function CreateListing() {
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      if (hasUnsavedChanges) {
        // Re-push current state to counteract the pop state and keep user on the page
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-purple-100 dark:from-gray-950 dark:to-gray-900 min-h-screen py-10 px-2 md:px-8 transition-colors duration-300">
      <div className="max-w-4xl mx-auto bg-white dark:bg-gray-900 rounded-xl shadow-lg dark:shadow-blue-900/10 p-6 relative transition-colors">
        ...
        {/* Verification Notice Banner - Compulsory for all creators */}
        <div className="mb-6 bg-yellow-50 dark:bg-yellow-950/20 border-l-4 border-yellow-400 p-4 rounded-lg shadow-md transition-colors">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4 className="text-sm font-bold text-yellow-800 dark:text-yellow-450 mb-1">⚠️ Property Verification Notice</h4>
              <p className="text-sm text-yellow-700 dark:text-gray-300">
                Upon creation, your property will be saved as <strong>unverified</strong> and <strong>will not be visible to buyers/tenants</strong>. You must complete the verification process after creation to publish it and start receiving inquiries.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className={`w-full p-3 border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors ${!formData.offer ? 'bg-gray-100 dark:bg-gray-950 cursor-not-allowed opacity-75' : ''}`}
                />
                ...
                <div key={index} className="space-y-2">
                  <div className="flex flex-col md:flex-row gap-2 md:items-center">
                    <input
                      ...
                      className={`w-full md:flex-1 p-3 border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors ${imageErrors[index] ? 'border-red-500' : ''
                        }`}
                    />
                    <input
                      type="text"
          ...
          {currentUser.role !== 'admin' && currentUser.role !== 'rootadmin' && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <label htmlFor="consent" className="text-sm text-gray-700 dark:text-gray-300 select-none cursor-pointer transition-colors">
                I confirm that <span className="font-semibold text-blue-700 dark:text-blue-400">all the information provided in this listing is true and genuine to the best of my knowledge</span>, and I agree to abide by the <Link to="/user/terms" target="_blank" className="underline font-bold text-blue-600 dark:text-blue-450 hover:text-blue-700">Terms and Conditions</Link>. Providing false information may result in account suspension or legal action.
              </label>
            </div>
          )}
      ...
      {showSecurityInfo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fadeIn">
          <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl relative text-left">
            ...
            <div className="space-y-3 text-sm text-slate-650 dark:text-slate-300 max-h-[60vh] overflow-y-auto pr-2">
              <p>
