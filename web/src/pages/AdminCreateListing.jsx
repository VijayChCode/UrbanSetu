...
export default function AdminCreateListing() {
  // Set page title
  usePageTitle("Create Property - Admin Panel");
  ...
  // Email assignment related states
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error('Failed to fetch users for autocomplete');
      }
    } catch (error) {
  ...
  }, [formData.assignToEmail, allUsers]);
  ...
  const onSubmitForm = async (e) => {
    ...
    if (formData.assignToEmail.trim()) {
      const isEmailValid = await validateEmail(formData.assignToEmail);
      if (!isEmailValid) {
      ...
      <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 relative transition-colors duration-300">
        ...
        <form onSubmit={onSubmitForm} className="space-y-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800 transition-colors duration-300">
            <h4 className="font-semibold text-blue-800 dark:text-blue-300 mb-3">User Assignment</h4>
            <div className="relative">
              <label htmlFor="assignToEmail" className="block text-sm font-medium text-blue-700 dark:text-blue-300 mb-2">
                Assign to (User Email - optional)
              </label>
              <input
                type="email"
                id="assignToEmail"
                placeholder="Enter user email to assign listing (leave empty for admin ownership)"
                onChange={onHandleChanges}
                value={formData.assignToEmail}
                className={`w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white transition-colors duration-300 ${emailError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
              ...
              <p className="text-sm text-blue-600 dark:text-blue-400 mt-2">
                💡 Tip: Start typing to see email suggestions. If left empty, the listing will be owned by the admin.
              </p>
            </div>
          </div>
                ...
                <div key={index} className="space-y-2">
                  <div className="flex flex-col md:flex-row gap-2 md:items-center">
                    <input
                      ...
                      className={`w-full md:flex-1 p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white transition-colors duration-300 ${imageErrors[index] ? 'border-red-500' : ''
                        }`}
                    />
                    <input
                      type="text"
                ...
                <div key={index} className="space-y-2">
                  <div className="flex flex-col md:flex-row gap-2 md:items-center">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                      ...
                      >
                        {(url && isAuditing[`tour_${index}`]) ? <UrbanSetuSpinner size="sm" isBright={true} /> : <FaBrain />}
                      </button>
                      <button
                        type="button"
