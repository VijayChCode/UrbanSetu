# 🔧 Chat Concurrency Fix — Mongoose VersionError & Server Crash

## Problem

When both buyer and seller send messages concurrently, the server crashes with:
```
VersionError: No matching document found for id "69bcc67efa921f2670a13b77" version 50
```
This escalates into an infinite error loop → memory leak → **FATAL ERROR: Reached heap limit Allocation failed - JavaScript heap out of memory** → server crash.

## Root Cause Analysis

> [!CAUTION]
> The **fundamental issue** was using the `findById → modify → save()` pattern in concurrent socket handlers. Mongoose uses **optimistic concurrency** (`__v` version key), so when two operations read version 50 and one saves first (version becomes 51), all others fail.

### Three problematic code paths identified:

| Location | Pattern | Frequency | Severity |
|---|---|---|---|
| `userAppointmentsActive` socket handler | `find → forEach → save()` | **Every 1 second per user** | 🔴 Critical |
| `messageReceived` socket handler | `findById → modify → save()` | Per message received | 🟠 High |
| `markCommentsAsRead` PATCH route | `findById → forEach → save()` with retry | Per chat open | 🟡 Medium |
| Unread counter update in comment POST | `findById → modify → save()` | Per message sent | 🟠 High |

### The Fatal Cascade:
```mermaid
flowchart TD
    A["Client emits userAppointmentsActive every 1s"] --> B["Server fetches ALL bookings for user"]
    B --> C["Server modifies comments in memory"]
    C --> D["Server calls .save() on each booking"]
    D --> E{"Concurrent .save() on same doc?"}
    E -->|Yes| F["VersionError thrown"]
    F --> G["Error logged but not handled"]
    G --> H["Next 1s interval fires"]
    H --> B
    F --> I["Error objects accumulate in memory"]
    I --> J["💥 OOM Crash"]
```

## Fixes Applied

### 1. Backend: Socket Handlers → Atomic Operations
**File:** [booking.route.js](file:///d:/Videos/Project/UrbanSetu/api/routes/booking.route.js#L3636-L3738)

```diff:booking.route.js
...
**`userAppointmentsActive`:** Replaced `find → forEach → save()` with:
- `booking.updateMany()` + `arrayFilters` — single atomic DB operation
- 500ms debounce to prevent rapid re-triggering

**`messageReceived`:** Replaced `findById → modify → save()` with:
- `booking.updateOne()` + `arrayFilters` — single atomic DB operation  
- Per-appointment concurrency lock

### 2. Backend: Unread Counter Update → Atomic `$inc`
**File:** [booking.route.js](file:///d:/Videos/Project/UrbanSetu/api/routes/booking.route.js#L703-L740)

Replaced `findById → modify → save()` with:
- `booking.findOneAndUpdate()` + `$inc` for atomic counter increment
- `booking.updateOne()` + `$set` for email timestamp

### 3. Backend: Mark Comments as Read → Atomic Operations  
**File:** [booking.route.js](file:///d:/Videos/Project/UrbanSetu/api/routes/booking.route.js#L2357-L2432)

Replaced 130+ lines of `findById → forEach → save()` with retry logic with:
- `booking.updateOne()` + `arrayFilters` + `$set`/`$addToSet` — single atomic operation
- Separate atomic `updateOne` for resetting unread counters

### 4. Frontend: Reduced Polling Frequency
**File:** [MyAppointments.jsx](file:///d:/Videos/Project/UrbanSetu/web/src/pages/MyAppointments.jsx#L602-L611)

- Changed `userAppointmentsActive` interval from **1 second → 30 seconds**
- Added **immediate emit on mount** so delivery status registers right away

> [!IMPORTANT]
> The 1-second interval was the primary amplifier of the backend bug. With 2 users online, the server was processing ~120 `find → modify → save()` cycles per minute on the same documents.

## Why Atomic Operations Fix This

| Old Pattern | New Pattern |
|---|---|
| Read document (version 50) | Single atomic `updateOne` |
| Modify in application memory | MongoDB handles concurrency internally |
| Save (fails if version changed) | No version conflict possible |
| Need retry logic | No retry needed |

> [!TIP]
> Atomic operations (`updateOne`, `updateMany`, `findOneAndUpdate`) with `$set`, `$inc`, `$addToSet` and `arrayFilters` bypass Mongoose's `__v` versioning entirely because MongoDB handles the concurrency at the database level.
